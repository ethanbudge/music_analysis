...
//
// Used for both MuLan (M1) and CLAP (M2).
//
// Partial pooling
// ---------------
//   Genre LMC slopes:
//     β_lmc_genre[g] = β_lmc + σ_lmc_genre · z_g
//     Each genre learns its own LMC-popularity slope, pooled toward β_lmc.
//
...
//
//   Effective LMC slope for song i (artist j, genre g):
//     β_lmc_genre[g] + γ_lmc · orientation[j]
// =============================================================================
...
transformed parameters {
  ...
  for (i in 1:N) {
    int j = artist_id[i];
    int g = genre_id[i];
    real lmc_slope_i = beta_lmc_genre[g] + gamma_lmc * orientation[j];
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    real lmc_slope_i = beta_lmc_genre[genre_id[i]]
                       + gamma_lmc * orientation[artist_id[i]];
